...
public class CarSpawner2 : MonoBehaviour
{
    ...
    void Update()
    {
...

        {
            ...
            if (k == 1)
            {
                Vector3 carPos1 = new Vector3(Random.Range(-1.71f, -1.71f), transform.position.y, transform.position.z);
                Instantiate(cars[carNo], carPos1, transform.rotation);

            }
            if (k == 2)
            {
                Vector3 carPos2 = new Vector3(Random.Range(-0.55f, -0.55f), transform.position.y, transform.position.z);
                Instantiate(cars[carNo], carPos2, transform.rotation);

            }
            if (k == 3)
            {
                Vector3 carPos3 = new Vector3(Random.Range(0.61f, 0.61f), transform.position.y, transform.position.z);
                Instantiate(cars[carNo], carPos3, transform.rotation);

            }
            if (k == 4)
            {
                Vector3 carPos4 = new Vector3(Random.Range(1.74f, 1.74f), transform.position.y, transform.position.z);
